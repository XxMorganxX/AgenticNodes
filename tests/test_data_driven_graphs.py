...
class DataDrivenGraphTests(unittest.TestCase):
    ...
    def test_graph_store_deletes_built_in_graphs_from_user_catalog(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            store = GraphStore(self.services, path=Path(directory) / "graphs.json")
            ...
            remaining_ids = {graph["graph_id"] for graph in store.list_graphs()}
            self.assertNotIn("tool-schema-repair", remaining_ids)
            with self.assertRaises(KeyError):
                store.get_graph("tool-schema-repair")
    ...
    def test_envelope_display_node_can_complete_without_outgoing_execution_edges(self) -> None:
        payload: dict[str, Any] = {
            ...
            "name": "Display Without Outgoing Edge",
            "description": "",
            "version": "1.0",
            "start_node_id": "start",
            "nodes": [
        ...
        state = runtime.run(graph, "hello display")

        self.assertEqual(state.status, "completed")
        self.assertIsNone(state.final_output)
        self.assertEqual(state.visit_counts["display"], 1)
        self.assertIn("display", state.node_outputs)
        self.assertTrue(any(event.event_type == "run.completed" for event in state.event_history))
        self.assertFalse(any(event.event_type == "run.failed" for event in state.event_history))
